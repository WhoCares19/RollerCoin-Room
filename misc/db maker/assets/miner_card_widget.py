...
class MinerCardWidget(QtWidgets.QWidget):
    ...
    Handles 'None' values from the parser to prevent crashes during string operations.
    """
    ...
    def init_ui(self):
        ...
        # Icon Overlay: Legacy Sign (Bottom Right)
        self.legacy_sign_label = QtWidgets.QLabel(self.image_container)
        self.legacy_sign_label.setFixedSize(30, 30)
        self.legacy_sign_label.setAlignment(QtCore.Qt.AlignCenter)
        self.legacy_sign_label.move(self.image_container.width() - 35, self.image_container.height() - 35) 
        self.legacy_sign_label.setStyleSheet("background-color: transparent; border: none;")
        self.legacy_sign_label.hide()
    ...
    def _get_max_levels(self):
        count = 0
        for i in range(1, 7):
            if f"level_{i}" in self.miner_data: count += 1
            else: break 
        return count
    ...
    def _cycle_level(self, event):
        ...
        if max_levels > 1:
            self.current_level_index = (self.current_level_index + 1) % max_levels
            self._update_display()
    ...
    def _update_display(self):
        ...
        for i in range(1, 7):
            if f"level_{i}" in self.miner_data:
                existing_levels.append(f"level_{i}")
        
        if not existing_levels:
            return

        if self.current_level_index >= len(existing_levels):
            self.current_level_index = 0
            
        current_level_key = existing_levels[self.current_level_index]
        current_level_data = self.miner_data.get(current_level_key)

        if current_level_data:
            p_val = current_level_data.get('raw_power') or "0.000"
            b_val = current_level_data.get('bonus') or "0.00"
            self.power_label.setText(f"{p_val}")
            self.bonus_label.setText(f"{b_val}%")
            self.level_indicator_label.setText("") 
        else:
            self.power_label.setText("N/A")
            self.bonus_label.setText("N/A")

        raw_level_path = current_level_data.get("level_icon") if current_level_data else None
        ...
        if level_img_path and os.path.exists(level_img_path):
            ...
            if not pixmap.isNull():
                scaled_pixmap = pixmap.scaled(self.level_indicator_label.size(), QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
                self.level_indicator_label.setPixmap(scaled_pixmap)
            else:
                self.level_indicator_label.setPixmap(QtGui.QPixmap())
                self.level_indicator_label.setText(current_level_key.split('_')[1])
        else:
            self.level_indicator_label.setPixmap(QtGui.QPixmap())
            self.level_indicator_label.setText(current_level_key.split('_')[1])

        # Set Sign Logic (Top Left) - Safe handling of None
        ...
        # Legacy Sign Logic (Bottom Right) - Safe handling of None
        is_legacy = (self.miner_data.get("is_legacy") or "").strip()
        raw_legacy_sign_path = self.miner_data.get("legacy_sign_icon_path") or ""
        legacy_sign_icon_path = self._resolve_path(raw_legacy_sign_path)

        if is_legacy.lower() == "yes" and legacy_sign_icon_path and os.path.exists(legacy_sign_icon_path):
            pixmap = QtGui.QPixmap(legacy_sign_icon_path)
            if not pixmap.isNull():
                scaled_pixmap = pixmap.scaled(self.legacy_sign_label.size(), QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
                self.legacy_sign_label.setPixmap(scaled_pixmap)
                self.legacy_sign_label.show()
            else:
                self.legacy_sign_label.hide()
        else:
            self.legacy_sign_label.hide()
