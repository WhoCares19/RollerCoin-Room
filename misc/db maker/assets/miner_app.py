...
class MinerApp(QtWidgets.QMainWindow):
    ...
    Features integrated 'Export Missing' buttons and unified level handling.
    """
    def __init__(self):
        super().__init__()
    ...
    def init_ui(self):
        ...
        self.btn_miner_csv = self._create_sidebar_btn("Import Miner CSV", self.import_csv)
        self.btn_miner_img = self._create_sidebar_btn("Import Miner Images", self.import_miner_images)
        self.btn_lvl_icons = self._create_sidebar_btn("Import Level Icons", self.import_level_icons)
        self.btn_miner_sets = self._create_sidebar_btn("Import Set Signs", self.import_set_sign_image)
        self.btn_miner_legacy = self._create_sidebar_btn("Import Legacy Signs", self.import_legacy_sign_image)
        
        for btn in [self.btn_miner_csv, self.btn_miner_img, self.btn_lvl_icons, self.btn_miner_sets, self.btn_miner_legacy]:
            sidebar_layout.addWidget(btn)

        # Racks
        sidebar_layout.addWidget(self._create_sidebar_header("RACKS"))
        self.btn_rack_csv = self._create_sidebar_btn("Import Racks CSV", self.import_rack_csv)
        self.btn_rack_img = self._create_sidebar_btn("Import Racks Images", self.import_rack_images)
        self.btn_rack_sets = self._create_sidebar_btn("Import Set Signs", self.import_set_sign_image)
        
        for btn in [self.btn_rack_csv, self.btn_rack_img, self.btn_rack_sets]:
            sidebar_layout.addWidget(btn)

        # Sets
        sidebar_layout.addWidget(self._create_sidebar_header("SETS"))
        self.btn_import_sets = self._create_sidebar_btn("Import Sets", self.import_sets_csv)
        sidebar_layout.addWidget(self.btn_import_sets)

        # Export
        sidebar_layout.addWidget(self._create_sidebar_header("EXPORT"))
        self.btn_export = QtWidgets.QPushButton("Export to SQL")
        self.btn_export.setObjectName("exportBtn")
        self.btn_export.clicked.connect(self.export_to_sql_action)
    ...
    def import_csv(self):
        ...
        if data:
            ...
            self._mark_done(self.btn_miner_csv)
    ...
    def import_rack_csv(self):
        ...
        if data:
            ...
            self.display_all_cards()
            self._mark_done(self.btn_rack_csv)
    ...
    def import_sets_csv(self):
        ...
        if data:
            ...
            self._mark_done(self.btn_import_sets)
    ...
    def import_miner_images(self):
        ...
        for miner in self.miners_data:
            ...
            for i in range(0, 7):
                lvl_key = f"level_{i}"
                if lvl_key in miner: 
                    keys.append(miner[lvl_key].get("level_id_tag", ""))
        ...
        self.display_all_cards()
        self._mark_done(self.btn_miner_img)
    ...
    def import_rack_images(self):
        ...
        self.display_all_cards()
        self._mark_done(self.btn_rack_img)
    ...
    def import_level_icons(self):
        ...
        self.display_all_cards()
        self._mark_done(self.btn_lvl_icons)
    ...
    def import_set_sign_image(self):
        ...
        self.display_all_cards()
        self._mark_done(self.btn_miner_sets)
        self._mark_done(self.btn_rack_sets)
    ...
    def import_legacy_sign_image(self):
        ...
        rel = self._get_rel_path(path)
        for miner in self.miners_data:
            # Check if Level 0 (Legacy Data) exists
            if "level_0" in miner:
                miner["level_0"]["level_icon"] = rel
        self.display_all_cards()
        self._mark_done(self.btn_miner_legacy)
    ...
    def export_to_sql_action(self):
        ...
        if success: 
            ...
            self._mark_done(self.btn_export)
