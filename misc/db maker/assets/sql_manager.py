...
def _init_db(cursor):
    ...
    Unified level structure: level_0 is used for legacy data.
    """
    ...
    # level_number 0 = Legacy, 1-6 = Standard
    cursor.execute('''
...
def export_to_sql(miners_data, racks_data, sets_data):
    ...
    Synchronizes Miners (Levels 0-6), Racks, and Sets.
    """
    ...
    try:
            ...
            if row:
                ...
                cursor.execute('''
                    ...
                    SET set_global_id = ?, legacy_id = ?, is_legacy = ?, image_path = ?, 
                        slot_size = ?, sets = ?, set_sign_icon_path = ?
                    WHERE id = ?
                ''', (set_global_id, legacy_id, is_legacy, image_path, slot_size, 
                      sets, set_sign, miner_id))
                ...
                ''', (m_name, set_global_id, legacy_id, is_legacy, image_path, 
                      slot_size, sets, set_sign))
                miner_id = cursor.lastrowid

            # Iterating 0-6 to include Legacy (0) and Standard (1-6)
            for i in range(0, 7):
        ...
        conn.commit()
        return True, "Catalog synced successfully."
